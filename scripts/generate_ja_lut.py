...
def export_faust_lib(
    ...
    with open(output_path, 'w') as f:
        ...
        f.write("import(\"ja_lut_common.lib\");\n\n")
        ...
        # Shared bilinear interpolation using the common helper
        f.write("// Bilinear interpolation lookups using shared helper\n")
        f.write(f"ja_lookup_m_end_{prefix}(m, h) = ja_lut_bilinear(\n")
        f.write(f"    ja_lut_{prefix}_m_end,\n")
        f.write(f"    ja_lut_{prefix}_m_min, ja_lut_{prefix}_m_max,\n")
        f.write(f"    ja_lut_{prefix}_h_min, ja_lut_{prefix}_h_max,\n")
        f.write(f"    ja_lut_{prefix}_m_size, ja_lut_{prefix}_h_size,\n")
        f.write("    m, h);\n\n")

        f.write(f"ja_lookup_sum_m_rest_{prefix}(m, h) = ja_lut_bilinear(\n")
        f.write(f"    ja_lut_{prefix}_sum_m_rest,\n")
        f.write(f"    ja_lut_{prefix}_m_min, ja_lut_{prefix}_m_max,\n")
        f.write(f"    ja_lut_{prefix}_h_min, ja_lut_{prefix}_h_max,\n")
        f.write(f"    ja_lut_{prefix}_m_size, ja_lut_{prefix}_h_size,\n")
        f.write("    m, h);\n")
