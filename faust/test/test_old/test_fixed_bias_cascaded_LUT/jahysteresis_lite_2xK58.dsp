// jahysteresis_lite.lib - 2x K58 Cascaded LUT version
// Jiles-Atherton model with cascaded 2D LUT (K116 = 2 × K58)
//
// 3 bias presets via ba.selectn (all compute, one selected):
// - Low:  bias_level=0.29 (subtle)
// - Mid:  bias_level=0.47 (balanced)
// - High: bias_level=0.74 (saturated)

import("stdfaust.lib");
import("ja_lut_k58_bias_low.lib");
import("ja_lut_k58_bias_mid.lib");
import("ja_lut_k58_bias_high.lib");
...
with {
  ...
  // 2x K58 CASCADE for each bias preset
  ja_hysteresis_low(H_audio) = (loop ~ _) : (!, _)
  with {
    loop(recM) = M_end, Mavg
    with {
      M1 = ja_lookup_m_end_k58_bias_low(recM, H_audio);
      sum1 = ja_lookup_sum_m_rest_k58_bias_low(recM, H_audio);
      M_end = ja_lookup_m_end_k58_bias_low(M1, H_audio);
      sum2 = ja_lookup_sum_m_rest_k58_bias_low(M1, H_audio);
  ...
  with {
    loop(recM) = M_end, Mavg
    with {
      M1 = ja_lookup_m_end_k58_bias_high(recM, H_audio);
      sum1 = ja_lookup_sum_m_rest_k58_bias_high(recM, H_audio);
      M_end = ja_lookup_m_end_k58_bias_high(M1, H_audio);
      sum2 = ja_lookup_sum_m_rest_k58_bias_high(M1, H_audio);
      Mavg = (sum1 + sum2) * inv_n;
    };
  };
