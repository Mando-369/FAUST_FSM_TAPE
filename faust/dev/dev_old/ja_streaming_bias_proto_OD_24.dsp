...
// Single mode: K60 only (no mode selection needed)
...
// ===== Streaming JA hysteresis (K60 only) =====
ja_hysteresis(H_in) = (loop ~ (mem, mem)) : (!, !, _) : *(inv_60)
with {
    loop(recM, recH) = recM, recH, H_in, phi_start, phase_span : ja_loop60;
};
