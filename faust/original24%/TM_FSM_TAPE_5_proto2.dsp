...
bias_resolution = hslider("Bias Resolution [K24|K48|K60]", 1.0, 0.0, 2.0, 1.0);
...
phase_cycles_from_mode(res) = ba.if(res < 0.5, 2.0, ba.if(res < 1.5, 3.0, 3.0));
...
ja_loop24(M_prev, H_prev, H_audio, phi_b, dphi_) =
  ...
  : ja_step_sc : ja_step_sc : ja_step_sc : ja_step_sc
  : ja_step_sc : ja_step_sc : ja_step_sc : ja_step_sc
  : ja_step_sc : ja_step_sc : ja_step_sc : ja_step_sc
  : ja_step_sc : ja_step_sc : ja_step_sc : ja_step_sc
  : ja_step_sc : ja_step_sc : ja_step_sc : ja_step_sc
...
ja_hysteresis(H_in) =
  ba.if(bias_resolution < 0.5,
    ...
    ba.if(bias_resolution < 1.5,
      ...
      loopK60(H_in)))
with {
  loopK24(H) = (loop ~ (mem, mem))
    : ba.selector(0, 3)
    : *(inv_24)
  with {
    loop(recM, recH) = recM, recH, H, phi_start, phase_span : ja_loop24;
  };
  ...
  loopK48(H) = (loop ~ (mem, mem))
    ...
    : *(inv_48)
  with {
    loop(recM, recH) = recM, recH, H, phi_start, phase_span : ja_loop48;
  };
  ...
  loopK60(H) = (loop ~ (mem, mem))
    ...
    : *(inv_60)
  with {
    loop(recM, recH) = recM, recH, H, phi_start, phase_span : ja_loop60;
  };
};
